...
const Plans = () => {
  const planFeatures = [
    ...
    },
    {
      ...
      features: [
        ...
        "Unlimited users and roles (Admins, Contributors, Viewers, Auditors)",
        "Access to selected compliance frameworks",
        "Limits on number of requests (expandable)",
        "Limits on document storage limits (expandable)"
      ],
    ...
    },
    {
      ...
      features: [
        "Multi-entity management from one platform",
        "Unlimited AI agent interactions",
        "Unlimited document storage",
        "Full access to all compliance frameworks",
  ...
  return (
    <PageTemplate
      title="Plans"
      description="Scalable plans for every stage of growth."
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
        {planFeatures.map((plan) => (
          <Card 
            key={plan.name} 
            className={`p-6 border ${plan.highlighted ? 'border-primary shadow-lg relative overflow-hidden' : 'border-slate-200'} h-full flex flex-col card-hover`}
          >
            {plan.highlighted && (
